...
<body>
    ...
    {{-- For the bookmark --}}
    <div class="nav-links">
        <span class="text-left">{{ $amendedAct['section'] }}
            {{-- @if (Route::has('login'))
                @auth                        
                        <a class="bookmarking" href="javascript:;" rel="/bookmarks/{{$amendedAct['post_act']}}/{{$amendedAct['section']}}/{{$amendedAct['id']}}/{{ Auth::user()->name }}/{{ Auth::user()->id }}/{{ Auth::user()->id }}{{$amendedAct['section']}}/{{$amendedAct['act_group']}}/{{$amendedAct['act_id']}}">
