...
<body>
    ...
    <div class="header_only" style="border: .1px solid #ddd;">
        <p style="padding-top: 7px; padding-bottom: .1px; padding-left: 16px; padding-right: 16px;"><b>{{ $allExecutiveAct['section'] }}</b>
            @if (Route::has('login'))
                @auth                        
                        <a class="bookmarking" href="javascript:;" rel="/bookmarks/{{$allExecutiveAct['executive_act']}}/{{$allExecutiveAct['section']}}/{{$allExecutiveAct['id']}}/{{ Auth::user()->name }}/{{ Auth::user()->id }}/{{ Auth::user()->id }}{{$allExecutiveAct['section']}}/{{$allExecutiveAct['executive_group']}}/{{$allExecutiveAct['executive_act_id']}}">
